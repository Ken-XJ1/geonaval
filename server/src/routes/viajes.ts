...
router.get('/mi-viaje', async (req: Request, res: Response) => {
  ...
  const rows = await safeQuery(
    `SELECT v.*, vp.asiento, vp.precio_pagado, vp.metodo_pago, e.nombre AS embarcacion_nombre,
            e.capacidad_pasajeros,
            (SELECT COUNT(*)::int FROM viaje_pasajeros WHERE viaje_id = v.id) AS pasajeros_count
...
router.get('/compras/stats', async (_req: Request, res: Response) => {
  try {
    ...
    const [ventasHoy, totalRecaudado, confirmados, pendientes] = await Promise.all([
      pool.query(
        `SELECT COUNT(*)::int AS count FROM viaje_pasajeros vp
         INNER JOIN pasajeros p ON p.id = vp.pasajero_id
         WHERE p.created_at >= $1 AND p.created_at <= $2`,
        [hoyInicio.toISOString(), hoyFin.toISOString()]
      ),
      pool.query(
        `SELECT COALESCE(SUM(precio_pagado), 0)::numeric AS total FROM viaje_pasajeros`
      ),
      pool.query(
        `SELECT COUNT(*)::int AS count FROM viaje_pasajeros vp
         INNER JOIN viajes v ON v.id = vp.viaje_id
    ...
    console.error('Compras stats:', (err as Error).message);
...
router.post('/:id/inscribir', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO viaje_pasajeros (viaje_id, pasajero_id, usuario_id, asiento, precio_pagado, metodo_pago)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [viajeId, pasajeroId, user.id, asiento, viaje.precio || 0, metodoPagoFinal]
    );
...
router.delete('/:id/cancelar-inscripcion', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const viajeDetalle = await pool.query(
      'SELECT origen, destino, fecha_salida FROM viajes WHERE id = $1',
      [viajeId]
    );
...
router.post('/:id/tripulacion', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await pool.query(
      ...
      [req.params.id, tripulante_id]
    );
    const operador = await pool.query(
      `SELECT t.nombre FROM tripulacion t WHERE t.id = $1`,
      [tripulante_id]
    );
    ...
    console.error('Asignar tripulación:', (err as Error).message);
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error('Error validando fecha:', (err as Error).message);
...
router.put('/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const viajeAnterior = await pool.query(
