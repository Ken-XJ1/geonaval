...
async function resolveUsuarioId(user) {
    if (user.email) {
        const byEmail = await pool_1.default.query('SELECT id FROM usuarios WHERE LOWER(email) = $1 AND activo = true', [user.email.toLowerCase()]);
        if (byEmail.rows[0])
            return byEmail.rows[0].id;
    }
    if (user.id > 0) {
        const byId = await pool_1.default.query('SELECT id FROM usuarios WHERE id = $1 AND activo = true', [user.id]);
        if (byId.rows[0])
            return byId.rows[0].id;
    }
    return null;
}
router.get('/mi-reserva', async (req, res) => {
    const user = req.user;
...
router.post('/reservar', async (req, res) => {
    ...
    try {
       ...
       WHERE v.id = $1`, [viaje_id]);
        ...
        const existe = await pool_1.default.query(`SELECT 1 FROM viaje_pasajeros WHERE viaje_id = $1 AND usuario_id = $2`, [viaje_id, usuarioId]);
        if (existe.rows[0]) {
            return res.status(400).json({ error: 'Ya estás inscrito en este viaje' });
        }
        const usuario = await pool_1.default.query('SELECT nombre, email FROM usuarios WHERE id = $1', [usuarioId]);
        const u = usuario.rows[0];
        let pasajeroId;
        const pasExistente = await pool_1.default.query('SELECT id FROM pasajeros WHERE usuario_id = $1', [usuarioId]);
       ...
       VALUES ($1, $2, $3, $4, $5)`, [viaje_id, pasajeroId, usuarioId, asientoFinal, viaje.precio || 0]);
        const reserva = await pool_1.default.query(`SELECT v.*, vp.asiento, vp.precio_pagado, e.nombre AS embarcacion_nombre
       FROM viaje_pasajeros vp
       INNER JOIN viajes v ON v.id = vp.viaje_id
       LEFT JOIN embarcaciones e ON e.id = v.embarcacion_id
       WHERE vp.viaje_id = $1 AND vp.usuario_id = $2`, [viaje_id, usuarioId]);
